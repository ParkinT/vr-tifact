...
static public class ObjectPoolController
{
    ...
    internal class ObjectPool
    {
        ...
        {
            if ( _pool == null ) _pool = new HashSet<PoolableObject>();
        }
        ...
        internal void _SetAvailable( PoolableObject poolObj, bool b )
        {
            ...
            if ( b )
            {
                ...
                _RecursivelySetActiveAndSendMessages( poolObj.gameObject, false, poolObj );
            ...
            {
                _RecursivelySetActiveAndSendMessages( poolObj.gameObject, true, poolObj );

                poolObj.gameObject.name = poolObj._myPool._prefabPoolObj.name;
            }
        }

        internal void _RecursivelySetActiveAndSendMessages( GameObject obj, bool active, PoolableObject parentPoolObj )
        {
            if ( active ) obj.active = true;

            if ( parentPoolObj.sendAwakeStartOnDestroyMessage )
            {
                if ( active )
                {
                    obj.SendMessage( "Awake", null, SendMessageOptions.DontRequireReceiver );

                    if ( obj.active && // Awake could deactivate object
            ...
            for ( int i = 0; i < obj.transform.childCount; i++ )
            {
                ...
                if ( active )
                {
                    _RecursivelySetActiveAndSendMessages( child.gameObject, active, parentPoolObj );
                }
                else
                {
                    var poolableChild = child.gameObject.GetComponent<PoolableObject>();

                    if ( poolableChild ) //if child is poolable itself it has to be detached and moved to the pool
                    {
            ...
            if ( !active )
            {
                obj.transform.parent = null; // object could still be parented, so detach
                obj.active = false;
            }  
        }
    }
...
public class PoolableReference<T>  where T : Component
{
    ...
    public T Get()
    {
        if ( _objComponent == null ) return null;
    ...
    {
        if ( componentOfPoolableObject == null )
        {
            Reset();
            return;
        }
